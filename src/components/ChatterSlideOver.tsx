...
/* Custom Tooltip */
function GlassTooltip({ active, payload, label, type }: any) {
  ...
  return (
    <div className="bg-zinc-900/90 backdrop-blur-2xl border border-white/[0.08] rounded-xl px-5 py-3.5 shadow-2xl">
      ...
      {type === "chats" ? (
        <>
          <p className="text-lg font-light text-blue-400">{row.open_chats} Offene Chats</p>
          <p className="text-xs text-white/40 font-light mt-1">{row.response_delay_days}d Verzug</p>
        </>
      ) : (
        <>
...
export default function ChatterSlideOver({ open, onClose, chatterName, platform }: Props) {
  ...
  const avgDelay = history.length ? (history.reduce((s, r) => s + r.response_delay_days, 0) / history.length).toFixed(1) : "0";
  ...
  const kpis = [
    ...
    { label: "Ø Antwort-Verzug", value: `${avgDelay}d`, gold: false },
  ];
          ...
          <div className="w-[480px] p-8 space-y-8">
            ...
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs text-white/25 mt-1 font-light tracking-wider">{platform} · Performance</p>
              </div>
              <button
                onClick={onClose}
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {kpis.map((kpi) => (
                    <div key={kpi.label} className="rounded-xl bg-white/[0.02] border border-white/[0.05] p-4">
                      ...
                      <p className={`text-2xl font-light mt-1.5 ${kpi.gold ? "gold-text" : "text-foreground/80"}`}>{kpi.value}</p>
                    </div>
                  ))}
                </div>
                ...
                <div className="rounded-2xl bg-white/[0.02] border border-white/[0.05] p-5">
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <LineChart data={history}>
                      ...
                      <Tooltip content={<GlassTooltip type="revenue" />} cursor={{ stroke: "rgba(212,175,55,0.15)" }} />
                ...
                {/* Open Chats Chart */}
                <div className="rounded-2xl bg-white/[0.02] border border-white/[0.05] p-5">
                  <p className="text-[10px] uppercase tracking-[0.2em] text-white/25 font-light mb-5">Offene Chats Verlauf</p>
                  <ResponsiveContainer width="100%" height={140}>
                    <LineChart data={history}>
                      <XAxis dataKey="analysis_date" tickFormatter={formatDate} axisLine={false} tickLine={false} tick={{ fill: "rgba(255,255,255,0.2)", fontSize: 10 }} />
                      <YAxis axisLine={false} tickLine={false} tick={{ fill: "rgba(255,255,255,0.15)", fontSize: 10 }} width={30} />
                      <Tooltip content={<GlassTooltip type="chats" />} cursor={{ stroke: "rgba(96,165,250,0.15)" }} />
                      <Line type="monotone" dataKey="open_chats" stroke="#60A5FA" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: "#60A5FA", stroke: "rgba(96,165,250,0.3)", strokeWidth: 6 }} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
                ...
                <div>
                  ...
                  <div className="rounded-xl bg-white/[0.015] border border-white/[0.04] overflow-hidden">
                    <div className="grid grid-cols-4 px-5 py-2.5 border-b border-white/[0.05]">
                      {["Datum", "Umsatz", "Chats", "DMs"].map((h) => (
                        <span key={h} className="text-[10px] uppercase tracking-[0.2em] text-white/20 font-light">{h}</span>
                      ))}
                    </div>
                    {[...history].reverse().map((row, i) => (
                      <div key={i} className="grid grid-cols-4 px-5 py-3 border-b border-white/[0.03] last:border-0 hover:bg-white/[0.01] transition-colors duration-300">
                        ...
                        <span className="text-sm text-white/40 font-light">{row.open_chats}</span>

...
export default function PeakRevenueCard() {
  ...
  const { perHour, daysObserved, totalRevenue, peakHour, nowHour } = useMemo(() => {
    const buckets = new Map<number, Map<string, number>>(); // hour → date → revenue
  ...
  const maxBar = Math.max(0.0001, ...perHour.map((p) => p.avg));

  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/[0.06] bg-gradient-to-br from-white/[0.03] via-white/[0.01] to-transparent p-5 shadow-[0_20px_60px_-30px_hsl(40_60%_50%/0.2),inset_0_1px_0_hsl(0_0%_100%/0.05)]">
      <div className="pointer-events-none absolute -top-20 -right-20 h-48 w-48 rounded-full bg-[hsl(40_60%_55%/0.08)] blur-3xl" />

      <div className="relative">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Euro className="h-3.5 w-3.5 text-white/45" />
        ...
        {loading ? (
          <p className="mt-6 text-center text-xs text-white/30 font-light">Lade Umsatzmuster…</p>
        ) : totalRevenue === 0 ? (
          <div className="mt-6 flex flex-col items-center gap-2 py-6 text-center">
            <Sparkles className="h-4 w-4 text-white/20" />
            <p className="text-xs text-white/40 font-light max-w-[280px]">
              Noch kein Umsatz erfasst. Sobald deine Chatter heute Umsatz machen,
              entsteht hier dein Stunden-Profil.
            </p>
          </div>
        ) : (
          <>
            <div className="mt-4 flex items-baseline gap-2">
              <span
                className="font-extralight tabular-nums text-[40px] leading-none text-white/95"
            ...
            <div className="mt-5">
              <div className="flex items-end gap-[3px] h-20">
                {perHour.map((p) => {
                  ...
                  const isPeak = p.hour === peakHour.hour && p.avg > 0;
                    ...
                    <div key={p.hour} className="group relative flex-1 h-full flex items-end">
                      <div
                        className={`w-full rounded-t-[2px] transition-all ${
                          isPeak
                            ? "bg-gradient-to-t from-amber-400/80 to-amber-200/90"
                            : isNow
                            ? "bg-gradient-to-t from-sky-400/70 to-sky-200/90"
                            : "bg-gradient-to-t from-white/[0.18] to-white/[0.32]"
                        }`}
                        style={{ height: `${Math.max(p.avg > 0 ? 6 : 2, pct)}%` }}
                        title={`${String(p.hour).padStart(2, "0")}:00 · Ø ${p.avg.toFixed(0)} € (max ${p.max.toFixed(0)} €)`}
                      />
                      <div className="pointer-events-none absolute -top-7 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-background/95 border border-white/10 rounded px-1.5 py-0.5 text-[9px] text-white/80 whitespace-nowrap font-light tabular-nums z-10">
                        {String(p.hour).padStart(2, "0")}h · Ø {p.avg.toFixed(0)} €
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="mt-4 flex items-center gap-4 text-[9.5px] text-white/35 font-light">
              <span className="flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-amber-300/80" /> Peak-Umsatz
              </span>
              <span className="flex items-center gap-1.5">
                <span className="h-1.5 w-1.5 rounded-full bg-sky-300/80" /> Jetzt ({String(nowHour).padStart(2, "0")}h)
              </span>
              <span className="flex items-center gap-1.5">

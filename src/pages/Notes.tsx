...
export default function Notes() {
  ...
  const handleSetPassword = async () => {
    ...
    setPwConfirm("");
    setVerifying(false);
    toast({ title: "Passwort gesetzt", description: "Texte sind jetzt geschützt." });
  };
  ...
  const handleResetPassword = () => {
    if (!confirm("Passwort wirklich zurücksetzen? Du musst beim nächsten Öffnen ein neues setzen. Die Texte selbst bleiben erhalten.")) return;
        ...
        <div className="flex flex-col items-center text-center space-y-6">
          ...
          <div className="space-y-1">
            <h1 className="text-lg font-light text-foreground/80 tracking-wide">
              {isFirstTime ? "Passwort festlegen" : "Texte gesperrt"}
            </h1>
            <p className="text-xs text-white/30 font-light">
              {isFirstTime
                ? "Wähle ein Passwort um deine Texte zu schützen"
